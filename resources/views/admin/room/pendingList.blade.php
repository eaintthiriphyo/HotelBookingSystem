...
<div class="container pt-4">
    ...
    @if($highlightBooking)
        <tr style="background-color: #57b6ee;">
            ...
            <td><span class="badge bg-secondary">{{ $highlightBooking->room->room_number ?? '-' }}</span></td>
            ...
            <td>
                <form action="{{ route('admin.booking.changePending', $highlightBooking->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="room_id" value="{{ $highlightBooking->room->id ?? '' }}">
                    <button type="submit" name="status" value="booked" class="btn btn-warning btn-sm">Approve</button>
                    <button type="submit" name="status" value="cancle" class="btn btn-danger btn-sm">Reject</button>
                </form>
            </td>
        </tr>
    @endif
    ...
    @foreach ($pending as $bl)
        @if(!$highlightBooking || $bl->id != $highlightBooking->id)
            <tr>
                ...
                <td>{{ $bl->check_out ?? '-' }}</td>
                <td>
                    <form action="{{ route('admin.booking.changePending', $bl->id) }}" method="POST">
                        @csrf
                        @method('PUT')
