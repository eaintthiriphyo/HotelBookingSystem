            ...
            <div class="card ">
                ...
                <div class="card-body">
                    ...
                    <table class="table mt-3">
                        ...
                        @foreach($room as $r)
                        @if($r->is_avaliable=='avaliable')
                        <tr>
                            <td>{{$r->room_number}}</td>
                            <td>{{$r->room_type->room_type}}</td>

                            <td>
                                <form action="{{route('admin.room.avaliableListUpdate',$r->id)}}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <select name="is_avaliable" class="form-control" onchange="this.form.submit()">
                                        ...
                                        <option value="avaliable" {{$r->is_avaliable=='avaliable' ? 'selected':''}}>Available</option>
