                    ...
                    <div class="row g-2">
                        ...
                        <div class="col-5">
                            <select id="fullNrcTownship" class="form-control">
                                ...
                                <option value="{{ $item['name_en'] }}">{{ $item['name_mm'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-2">
        ...
        <form id="quickBookingForm" action="{{ route('admin.booking.store') }}" method="post" style="display:none;">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <div class="row g-2">
                        <div class="col-3">
                            <select id="quickNrcCode" class="form-control">
                                <option value="">State/Region</option>
                                @for($i=1; $i<=14; $i++)
                                    <option value="{{ $i }}">{{ $i }}/</option>
                                    @endfor
                        ...
                        <div class="col-5">
                            <select id="quickNrcTownship" class="form-control">
                                <option value="">Township</option>
                                @foreach($nrcData['data'] as $item)
                                <option value="{{ $item['name_en'] }}">{{ $item['name_mm'] }}</option>
                                @endforeach
                        ...
                        <div class="col-2">
                            <select id="quickNrcType" class="form-control">
                                <option value="N">(N)</option>
                                <option value="E">(E)</option>
                                <option value="P">(P)</option>
                            </select>
                        ...
                        <div class="col-2">
                            <input type="text" id="quickNrcNumber" class="form-control" maxlength="6" placeholder="123456">
                        </div>
                    </div>
                    <input type="hidden" name="credential" id="quickCredential">
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        function generateNRC(formPrefix) {
            const code = $(`#${formPrefix}NrcCode`).val();
            const township = $(`#${formPrefix}NrcTownship`).val();
            const type = $(`#${formPrefix}NrcType`).val();
            const number = $(`#${formPrefix}NrcNumber`).val();

            if (code && township && type && number) {
                const result = `${code}/${township}(${type})${number}`;
                $(`#${formPrefix}Credential`).val(result);
            }
        }
        $('#fullBookingForm, #quickBookingForm').on('submit', function(e) {
            const formPrefix = $(this).attr('id') === 'fullBookingForm' ? 'full' : 'quick';
            generateNRC(formPrefix);

        });

        $('#fullNrcCode, #fullNrcTownship, #fullNrcType, #fullNrcNumber').on('change keyup', function() {
            generateNRC('full');
        });
        $('#quickNrcCode, #quickNrcTownship, #quickNrcType, #quickNrcNumber').on('change keyup', function() {
            generateNRC('quick');
     ...
     // Filter Townships by NRC code
function filterTownships(formPrefix) {
    const code = $(`#${formPrefix}NrcCode`).val();
    const townshipSelect = $(`#${formPrefix}NrcTownship`);
    townshipSelect.empty();
    townshipSelect.append('<option value="">Township</option>');

    nrcData.forEach(item => {
        if (String(item.state) === String(code)) { // Make sure 'state' matches your data key
            townshipSelect.append(`<option value="${item.name_en}">${item.name_mm}</option>`);
        }
    });

    // Reset type and number
    $(`#${formPrefix}NrcType`).val('N');
    $(`#${formPrefix}NrcNumber`).val('');
}

// Attach change events **outside** the function
$(document).ready(function() {
    $('#fullNrcCode').change(function() { filterTownships('full'); });
    $('#quickNrcCode').change(function() { filterTownships('quick'); });
});
        ...
        $('#checkEmailBtn').click(function() {
            ...
            $.get("{{ route('admin.booking.checkUser') }}", {
                email: email
            }, function(data) {
                if (data.user) {
                    $('#quickBookingForm').show();
        ...
        let selectedRoomType = {
            fullBookingForm: null,
            quickBookingForm: null
        };
