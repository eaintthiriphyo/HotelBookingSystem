            ...
            <div class="card-body">
                <table class="table  mb-0">
                    <tr>
                        ...
                        <td>
                            <h5>{{ \Carbon\Carbon::now()->format('Y-m-d') }}</h5>
                        </td>
                    </tr>
                </table>
            </div>
            ...
            <div class="col-md-8">
                ...
                <div class="card shadow-sm mb-4 border-0" id="bookingStep" style="border-radius:12px;">
                    ...
                    <div class="card-header text-white"
                        style="background-color: navy; border-top-left-radius:12px; border-top-right-radius:12px;">
                        ...
                        <div class="mb-4">
                            ...
                            <div id="roomTypeButtons" class="d-flex flex-wrap gap-2 mt-2">
                                @foreach ($roomTypes as $type)
                                    <button type="button" class="room-type-btn " data-id="{{ $type->id }}"
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="email" id="inputEmail" class="form-control form-control-lg"
                                    placeholder="example@gmail.com" name="email" value="{{ Auth::user()->email }}">
                                @error('email')
                                    <p class="text-danger">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" id="inputPhone" class="form-control form-control-lg"
                                    placeholder="09xxxxxxxxx" name="phone" value="{{ Auth::user()->phone }}" required>
                                @error('phone')
                                    <p class="text-danger">{{ $message }}</p>
                                @enderror
                            </div>
                                ...
                                <div class="row g-2">
                                    ...
                                    <div class="col-2">
                                        <select id="fullNrcType" class="form-control">
                                            <option value="N" {{ old('nrc_type') == 'N' ? 'selected' : '' }}>(N)
                                            </option>
                                            <option value="E" {{ old('nrc_type') == 'E' ? 'selected' : '' }}>(E)
                                            </option>
                                            <option value="P" {{ old('nrc_type') == 'P' ? 'selected' : '' }}>(P)
                                            </option>
                                        </select>
                                    </div>
                                    <div class="col-2">
                                        <input type="text" id="inputCredential" class="form-control" maxlength="6"
                                            placeholder="123456" value="{{ old('nrc_number') }}">
                                    </div>
                                    @error('credential')
                            ...
                            <div class="col-12">
                                ...
                                <textarea id="inputAddress" rows="2" name="address" class="form-control form-control-lg" required
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateCheckOutMin() {
                ...
                if ($('#checkOut').val() < $('#checkOut').attr('min')) $('#checkOut').val($('#checkOut').attr(
                    'min'));
            }
            updateCheckOutMin();
    ...
    btn.addClass('active');
                ...
                if (images && images.length > 0) {
                    images.forEach((img, index) => {
                        let imgUrl = img.img_src.startsWith('http') ? img.img_src :
                            `{{ asset('') }}${img.img_src}`;
                        carousel.append(
                            `<div class="carousel-item ${index === 0 ? 'active' : ''}"><img src="${imgUrl}" class="d-block w-100" style="height:265px; object-fit:cover;"></div>`
                        );
                    });
                } else {
                    carousel.append(
                        `<div class="carousel-item active"><img src="https://via.placeholder.com/500x300" class="d-block w-100"></div>`
                    );
                }
            ...
            function fetchAvailableRooms() {
                ...
                $.get("{{ route('user.booking.availableRooms') }}", {
                    ...
                    data.rooms.forEach((room, index) => {
                        ...
                        container.append(
                            `<button class="btn ${activeClass} me-2 mb-2 room-btn" data-id="${room.id}">${room.room_number}</button>`
                        );
                        if (index === 0) {
